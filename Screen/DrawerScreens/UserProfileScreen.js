...
const ProfileScreen = () => {
  ...
  const fetchData = () => {
    fetch('http://localhost:3000/residents/list_resident?user_id=' + userId ,
    {
    	method: "GET",
        headers: {
    ...
    const handleSubmitButton = () => { 
      ...
      if (!userPhone) {
        alert('Please fill a phone');
        return;
      }
      if (!userAge) {
        alert('Please fill an age');
        return;
      } 
      if (!userSIN) {
        alert('Please fill a SIN');
        return;
      }
      setLoading(true);
      fetch('http://localhost:3000/residents/modify_resident?user_id=' + userId + '&completename=' + userName + '&address=' + userAddress + 
